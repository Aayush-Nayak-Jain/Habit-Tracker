...
import {todayKey, last90Days, currentWeekKeys, lastNDays, calcStreek} from "../utils/dateHelpers.js";
...
export const markComplete = async (req, res) => {
    try{
        ...
        const log = await HabitLog.findOneAndUpdate(
            {userId: req.user._id, habitId, date: completedDate},
            {$setOnInsert: {userId: req.user._id, habitId, date: completedDate}},
...
export const getHabitStats = async (req, res) => {
    try{
        ...
        const {current , longest }= calcStreek(dateKeys);
...
export const getAllStats = async (req, res) => {
    try{
        ...
        const perHabit = habits.map((h)=> {
            ...
            const {current, longest} = calcStreek(keys);
            return {
                habitId: h._id,
                name: h.name,
                icon: h.icon,
